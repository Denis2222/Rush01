    ...
    public class FireProjectileScript : FireBaseScript, ICollisionHandler
    {
        ...
        public void HandleCollision(GameObject obj, Collision c)
        {
            ...
            if (c.contacts.Length != 0)
            {
                ...
                print("HandleCollision Projectile");
                if (c.gameObject.tag == Constants.ENEMY_TAG)
                {
                    print("ENEMYTAG "+c.gameObject.tag+ " "+ c.gameObject.name);
                    EnemyController controller = c.gameObject.GetComponent<EnemyController>();
